...
> Stack abaixo reflete [ADR-001](adr/ADR-001-stack.md), **aprovado pelo CEO em 2026-08-03**. Este doc é atualizado a cada mudança estrutural relevante (regra de doc viva). **Atualizado em 2026-08-21 após Sprint 34** — Redesign visual "Analyst Console": novo sistema de design (namespace tokens `--ac-*` coexistindo com o antigo), tipografia Inter self-hosted, shell/sidebar redesenhado, Dashboard topo completamente migrado (5 KPIs com delta+sparkline via `KpiTile`, tabela de conferência Saldo Acumulado sempre visível, gráfico combinado Receita/Despesa/Saldo Acumulado com tooltip funcional, navegador de mês). Frontend agora com cobertura de testes configurável via `@vitest/coverage-v8` com thresholds por arquivo (80% em lógica de negócio — primeira vez neste projeto). Componentes novos reutilizáveis: `KpiTile`, `ChartTooltip` (tooltip/crosshair pra gráfico de série única, ainda não consumido fora de si mesmo — reservado pro épico E10), `sharedChartDomain`. Ver seção própria abaixo e [DESIGN.md](../../DESIGN.md) seção "Analyst Console".
