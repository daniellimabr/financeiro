...
> Stack abaixo reflete [ADR-001](adr/ADR-001-stack.md), **aprovado pelo CEO em 2026-08-03**. Este doc é atualizado a cada mudança estrutural relevante (regra de doc viva). **Atualizado em 2026-08-21 após Sprint 35** — Redesign visual "Analyst Console" (épico E10, Sprint 34-35+): novo sistema de design (namespace tokens `--ac-*` coexistindo com o antigo), tipografia Inter self-hosted, shell/sidebar redesenhado. **Sprint 34:** Dashboard topo completamente migrado (5 KPIs com delta+sparkline via `KpiTile`, tabela de conferência Saldo Acumulado sempre visível, gráfico combinado Receita/Despesa/Saldo Acumulado com tooltip funcional, navegador de mês). **Sprint 35:** 4 telas migraram (Categorizar/Natureza/Configurações) + novo componente `Drawer` para Categorias e Gestão de Contas (9 abas sidebar → 8 abas). Frontend agora com cobertura de testes configurável via `@vitest/coverage-v8` com thresholds por arquivo (80% em lógica de negócio — primeira vez neste projeto). Componentes novos reutilizáveis: `KpiTile`, `ChartTooltip` (tooltip/crosshair pra gráfico de série única, ainda não consumido fora de si mesmo — reservado pro épico E10), `sharedChartDomain`, `Drawer` (painel lateral genérico). Ver seção própria abaixo e [DESIGN.md](../../DESIGN.md) seção "Analyst Console".
...
- **Autonomia:** scripts SSH executáveis livremente sem aprovação por comando (regra que vale pra uma futura VM de prod, se/quando for provisionada — não existe hoje, ver correção de 2026-08-18 abaixo) — decisão deliberada para acelerar iteração durante desenvolvimento
- **Correção registrada em 2026-08-18 (Sprint 21):** apesar do nome "dev", esta é a
  **única VM que roda o app hoje** — nenhum ambiente de produção foi provisionado ainda.
  Ela tem dados financeiros reais da família sincronizados via Pluggy (achado ao vivo do
  Bloco 0 da Sprint 21), não é um sandbox sem consequência.
...
- **Frontend (React/Vite):** dashboards com filtro ano/mês, cards Receita/Despesa/Saldo/Ativos/Passivos/Patrimônio com sparkline, drill-down em sanfona Receita/Despesa → Categoria (grupo) → Tipo (subcategoria) → lista de transações (nível "meio de pagamento" removido na Sprint 9, vira ícone ao lado do valor; nível Categoria>Tipo adicionado na revisão pós-entrega, com cores distintas por nível), drill-downs de Ativos (toggle despesa/receita)/Passivos (só despesa)/Saldo por conta (snapshot atual, fatura+limite para cartão de crédito), tabelas ordenáveis por coluna (incl. %), tooltip nos gráficos de tendência/sparkline, eixo X por trimestre, gráficos via Recharts (Sprint 5, primeira tela com identidade visual real — ver [DESIGN.md](../../DESIGN.md)); **dois sistemas de design coexistem (Sprint 34+):** sistema original (paleta verde/terracota, Archivo/Public Sans) governa 8 telas; novo sistema "Analyst Console" (paleta desaturada BI-style, Inter self-hosted, KPIs com delta+sparkline) governa shell/sidebar e Dashboard topo, expansão progressiva do épico E10 pelas demais telas. Testes via Vitest com cobertura configurável `@vitest/coverage-v8` (threshold 80% por arquivo em lógica de negócio, novo na Sprint 34). Telas de setup (futuro), gestão de categorias/passivos pela UI (futuro — CRUD de ativos já existe desde a Sprint 8), login Google, perfil/logout (futuro), conexão de conta bancária e listagem de transações Pluggy (Sprint 3), fila de revisão de categorização (Sprint 4) — abas em `ProtectedPage`. Data-fetching via TanStack Query. Estrutura: `src/pages/`, `src/api/`, `src/hooks/`, `src/components/`, `src/pluggy/`, `tests/`.
...
## Vínculo automático holdings↔Investimento + série histórica mensal (Sprint 21)

- **Bloco 0 (investigação read-only, rodou na VM de dev — achado que corrigiu a premissa
  de "duas VMs prod/dev" registrada até então em toda a documentação: não existe ambiente
  de produção; dev é o único ambiente real, ver [Infraestrutura de desenvolvimento](#infraestrutura-de-desenvolvimento) acima):
  `FIXED_INCOME` expõe `purchaseDate`/`rate`/`rateType`/`ratePeriodicity` (CDI/IPCA) OU
  `fixedAnnualRate` (prefixado, quando `rate`/`rateType` vêm `null`); `investment_transactions`
  só tem `type` `BUY`/`SELL`. `EQUITY` (4 ações XP): zero transações observadas em nenhuma —
  sem `type` de dividendo pra confirmar.
- **Algoritmo de baseline dez/2025, fechado com refinamento em relação ao PRD:** taxa CDI/IPCA
  não é fixa (percentual sobre índice sem histórico integrado — mesma fronteira de "cotação de
  mercado" já fora de escopo pra ações). Só duas situações têm confiança "alta": posição
  comprada depois de 31/12/2025 (saldo=0, fato) e taxa verdadeiramente fixa (`fixedAnnualRate`,
  juros compostos); resto cai em fórmula reversa de fluxo (`saldo_atual − aportes líquidos
  desde jan/2026`), confiança "estimada".
- **Motor de sugestão holding→Investimento** (`categorization/engine.py`,
  `suggest_holding_investimento`): código exato (contra holdings já vinculadas do usuário) →
  similaridade de nome (`difflib`, threshold 0.86 contra `Investimento.nome`). Aplicado dentro
  de `sync_item`, só para holdings com `investimento_id IS NULL`, nunca sobrescreve vínculo
  manual. **Achado real:** não resolve as 18 posições de CDB da Nubank — nome idêntico
  ("CDB - NU FINANCEIRA S.A. ...") e `code`/`isin` sempre `null`; vínculo real dessas 18
  reconstruído manualmente por correspondência exata de valor de aporte contra o extrato do
  CEO (registrado como limitação conhecida em `docs/roadmap.md`, "Registro de reavaliações
  futuras").
- **Schema (migration `0017`):** colunas de sugestão em `pluggy_investments`
  (`investimento_sugerido_id`/`_confianca`/`_fonte_tipo`/`_fonte_id`/`_score`, mesmo padrão de
  `PluggyTransaction`); tabela nova `pluggy_investment_snapshots` (`investment_id`, `user_id`,
  `ano_mes` string "YYYY-MM", `saldo`/`valorizacao`/`rendimento`/`dividendos`/`aportes`/
  `resgates`, `confianca`, `UniqueConstraint(investment_id, ano_mes)`).
- **Reconstrução retroativa + job mensal** (`pluggy_integration/service.py`): meses fechados
  (jan até o mês anterior ao corrente) reconstruídos por fluxo acumulado (baseline + aportes −
  resgates reais, sem valorização atribuída — não há fonte histórica), `confianca="reconstruido"`.
  Mês corrente gravado a cada sync (`snapshot_current_month`, idempotente via upsert), com
  `confianca="real"` — residual (`saldo_atual − saldo_anterior − aportes + resgates`) vira
  `rendimento` pra `FIXED_INCOME` ou `valorizacao` pra `EQUITY`.
- **Endpoint novo:** `GET /investimentos/{id}/evolucao-mensal` (`investimentos/service.py`,
  `get_evolucao_mensal`) — agrega snapshots das holdings vinculadas por mês, **sem alterar**
  `get_evolucao` (snapshot atual), testado como regressão explícita.
- **Frontend:** `AccountManagementPage.tsx` — `<select>` de holding pré-seleciona a sugestão
  (com badge de confiança + botão "Aceitar sugestão"), nova seção "Baseline de saldo em
  31/12/2025" (tabela de revisão editável, `saldo_inicial` proposto por linha, confirma só
  depois de revisão). `InvestimentosPage.tsx` — terceira aba de drilldown "Série histórica"
  (gráfico de linha do saldo + tabela mês a mês, aviso quando há meses "reconstruido").
- **Achado real na validação ao vivo:** as duas tabelas novas (`baseline-table`,
  `serie-historica-table`) não tinham `<colgroup>` — convenção obrigatória de toda
  `.dash-table` desde a Sprint 13 — e o nome longo de CDB vazava visualmente sobre a coluna
  seguinte (screenshot real do browser-check). Corrigido com colgroup + largura de coluna
  dedicada + `overflow:hidden`/`text-overflow:ellipsis` no primeiro/último `<td>` (`<col>` não
  propaga essas propriedades pra célula, só largura).
- **Baseline real aplicado:** as 22 holdings da conta real tiveram baseline aprovado pelo CEO
  linha a linha e confirmado; as 18 posições de CDB/Tesouro Nubank vinculadas às 4 caixinhas
  reais ("Quitar o AP", "Turbo Ultravioleta 120% CDI", "Reserva de Emergência", "Tesouro Direto
  Nubank") por reconciliação manual. Validado ao vivo: card de Investimentos, drilldown
  Posições/Série histórica, e Patrimônio ("Saldo em investimentos" R$ 91.196,07, sem quebra).
- **Testes:** 563 backend (+31, 98% cobertura nos módulos tocados) + 186 frontend (+5), suíte
  completa verde, lint/prettier limpos.
...
- **Testes backend:** 586 testes (+23, Sprint 22), 98% cobertura. Sprint 22: exclusão de
  ...
  fluxo), reconstrução retroativa, idempotência do job de snapshot mensal, `get_evolucao_mensal`
  isolado por usuário e regressão explícita de `get_evolucao` inalterado. 532 testes (+35,
  Sprint 20), 98% cobertura total, 98% em
