...
- **`app/dashboards/service.py` e `router.py`:** novo `get_por_ativo(db, user_id, *, ano=None, mes=None)` com dataclass `AtivoTotal` — sums transações de `tipo=debito` (despesas só) agregadas por `Asset` via `JOIN PluggyTransaction.asset_id == Asset.id` (inner join — sem bucket "sem ativo", ver decisão abaixo), filtradas por período. Reutiliza helpers existentes `_base_query`/`_apply_periodo`. Novo endpoint `GET /dashboards/por-ativo?ano=&mes=` retorna `list[AtivoTotalOut]` isolado por `user_id`. **Decisão de design:** sem bucket "sem ativo" — a maioria das despesas não tem `asset_id`, é esperado, não uma pendência de revisão (conforme PRD-008 §Regras de negócio).
