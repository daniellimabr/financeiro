...
> Stack abaixo reflete [ADR-001](adr/ADR-001-stack.md), **aprovado pelo CEO em 2026-08-03**. Este doc é atualizado a cada mudança estrutural relevante (regra de doc viva). **Atualizado em 2026-08-17 após Sprint 19** — Gestão de Investimentos, Blocos 1+2: modelo `Investimento` (agrupamento lógico de carteiras Pluggy), categorização de Aporte/Resgate (subcategorias normais, contam nos totais), tela `InvestimentosPage`. Bloco 3 (investigação com dado real) achou rendimento incidental via endpoint genérico já integrado, mas confirmou que a visão completa de holdings exige as rotas dedicadas de Investments da Pluggy — vira a próxima sprint, sem implementação nesta. Sprint sem épico prévio no roadmap. Ver seção própria abaixo.
...
## Qualidade (Sprint 1 → Sprint 19)

- **Testes backend:** 497 testes (+54), 98% cobertura total, 100% em
  `app/investimentos/`. Sprint 19: CRUD/isolamento/`get_evolucao` de
