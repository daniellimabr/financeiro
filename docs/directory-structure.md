...
Atualizado a cada mudança estrutural. Estado atual (fim da Sprint 19 — Gestão de Investimentos, Blocos 1+2: modelo `Investimento`, categorização de Aporte/Resgate, tela `InvestimentosPage`; Bloco 3 — investigação com dado real, sem implementação, integração completa de Investments da Pluggy vira a próxima sprint):
...
│   │   │   ├── service.py          # list/get/create/update/delete_investimento (delete desassocia carteiras E transações, dois caminhos); get_evolucao (saldo_base/saldo_atual/total_aportes/total_resgates/rendimento_estimado, só transações confirmadas)
│   │   │   └── router.py
...
│   │   │   └── router.py           # rotas /pluggy/*; PUT /pluggy/accounts/{id}, POST /pluggy/sync (Sprint 7); filtro liability_id (Sprint 9); PUT /accounts/{id}/saldo-inicial, GET/PUT /ajuste-salario-dezembro (Sprint 15); update_account +investimento_id (validado por user_id), filtro investimento_id em list_transactions (Sprint 19)
...
│   │   ├── test_investimento_service.py # CRUD/isolamento de Investimento; delete_investimento desassociando carteiras E transações (dois caminhos); get_evolucao (saldo base multi-carteira, aritmética de rendimento_estimado, só transações confirmadas) (Sprint 19, novo)
