...
Atualizado a cada mudança estrutural. Estado atual (fim da Sprint 34 — Redesign visual "Analyst Console": fundação + Dashboard):
...
│   │   └── PRD-030-categorias-por-usuario-orcamento-gestao-categorias.md  # Sprint 30 — categorias/subcategorias por usuário, Orçamento, Gestão de Categorias, remoção da Projeção (cross-epic, sem épico prévio)
│   ├── sprints/
...
│   │   ├── SPRINT-030-categorias-por-usuario-orcamento-gestao-categorias-plan.md  # Plano Sprint 30 (2026-08-20)
│   │   └── SPRINT-030-categorias-por-usuario-orcamento-gestao-categorias-report.md  # Relatório Sprint 30 (2026-08-20)
...
│   │   │   ├── pluggy.py           # PluggyItem/Account/Transaction + enums (Sprint 3; +9 colunas de categorização/ativo na Sprint 4; +apelido/sync_enabled em Account, +descricao_usuario/sugerida/origem_id em Transaction na Sprint 7; +liability_id/liability_sugerido_id/liability_sugestao_confianca, +@property account_tipo na Sprint 9; +limite_credito/fatura_vencimento em Account na revisão pós-entrega da Sprint 9; +saldo_inicial em Account na Sprint 15; +data_editada_manualmente em Transaction — trava contra sobrescrita em resync (Sprint 18); +investimento_id em Account, +investimento_id/investimento_sugerido_id/investimento_sugestao_confianca em Transaction (Sprint 19)); +PluggyInvestment (holdings), +PluggyInvestmentTransaction (ledger de posições) (Sprint 20); `INVESTIMENTO_PROVENTOS_CATEGORIAS_PLUGGY` (constante de Sprint 22, exclusão de proventos por categoria_pluggy) removida (Sprint 32)
│   │   │   ├── investimento.py     # Investimento — agrupamento lógico do usuário, só nome (Sprint 19, novo)
│   │   │   ├── orcamento.py         # Orcamento — vigência eventual (ano+mes exatos) ou recorrente (data_inicio + data_fim opcional), sem unicidade (múltiplos por subcategoria permitidos) (Sprint 30, novo)
│   │   │   └── categorization.py   # CategorizationRule — memória de mapeamento padrão→subcategoria (Sprint 4); AssetCategorizationRule — mirror pra ativo, trocando subcategory_id por asset_id (Sprint 10); InvestimentoCategorizationRule — mirror pra investimento, trocando subcategory_id por investimento_id (Sprint 19)
│   │   ├── schemas/
...
│   │   ├── categories/             # CRUD category_groups/subcategories, isolado por user_id (Sprint 2, dado global até Sprint 30; Sprint 30: por usuário com seed padrão congelado)
│   │   │   ├── service.py
│   │   │   ├── seed.py              # seed_categories_for_user — catálogo padrão congelado, snapshot da realidade em 2026-08-20 (Sprint 30, novo)
│   │   │   └── router.py
│   │   ├── orcamentos/              # CRUD Orçamento (eventual/recorrente), isolado por user_id (Sprint 30, novo)
│   │   │   ├── service.py
│   │   │   └── router.py
...
│   │   │   └── router.py
│   │   ├── investimentos/          # CRUD Investimento + get_evolucao, isolado por user_id (Sprint 19, novo); get_evolucao_mensal (Sprint 21); get_transacoes — extrato unificado, une PluggyTransaction (conta) + PluggyInvestmentTransaction (holding, join por PluggyInvestment.investimento_id), filtro ano/mes opcional (Sprint 23, novo)
│   │   │   ├── service.py          # list/get/create/update/delete_investimento (delete desassocia carteiras E transações, dois caminhos); get_evolucao (saldo_base/saldo_atual/total_aportes/total_resgates/rendimento_estimado, só transações confirmadas); get_evolucao soma também PluggyInvestment.saldo_inicial/valor_atual das holdings vinculadas, mesma fórmula de rendimento_estimado (Sprint 20)
│   │   │   └── router.py
│   │   ├── pluggy_integration/     # integração Pluggy — connect token, sync manual (Sprint 3)
│   │   │   ├── client.py           # PluggyClient — auth por API key cacheada, get_item/accounts/transactions; get_investments/get_investment_transactions + _get_paginated (paginação por page/totalPages, achado real do Bloco 1 — diferente do cursor de /v2/transactions) (Sprint 20)
│   │   │   ├── service.py          # register_item, sync_item, list_items/accounts/transactions; update_account, sync_items (Sprint 7); filtro liability_id, joinedload(account) em list_transactions p/ account_tipo sem N+1 (Sprint 9); _upsert_account persiste limite_credito/fatura_vencimento de creditData (revisão pós-entrega da Sprint 9); update_saldo_inicial, upsert_salario_ajuste_dez_2025/get_salario_ajuste_dez_2025 (transação sentinela) (Sprint 15); _parse_date converte UTC→America/Sao_Paulo antes de .date() (bug de fuso corrigido); _upsert_transaction/upsert_salario_ajuste_dez_2025 gravam data_competencia via competencia_padrao + data_caixa via caixa (Sprint 16); _upsert_transaction pula data/data_competencia/data_caixa quando data_editada_manualmente=True (Sprint 18); sync_item busca investments pra todo item (mesmo com contas); _upsert_investment/_upsert_investment_transaction (upsert idempotente, preserva investimento_id/saldo_inicial em resync); list/get/update_investment, update_investment_saldo_inicial, list_investment_transactions; _parse_investment_date — sem conversão de fuso, diferente de _parse_date (achado real: date/tradeDate de /investments/transactions são meia-noite UTC, "só a data") (Sprint 20)
│   │   │   └── router.py           # rotas /pluggy/*; PUT /pluggy/accounts/{id}, POST /pluggy/sync (Sprint 7); filtro liability_id (Sprint 9); PUT /accounts/{id}/saldo-inicial, GET/PUT /ajuste-salario-dezembro (Sprint 15); update_account +investimento_id (validado por user_id), filtro investimento_id em list_transactions (Sprint 19); GET /investments (filtro investimento_id), PUT /investments/{id}, PUT /investments/{id}/saldo-inicial, GET /investments/{id}/transactions (Sprint 20)
...
│   ├── package.json                # dependências frontend (React, TanStack Query, ESLint, Prettier, Vitest); script `test:coverage` novo (Sprint 34, @vitest/coverage-v8)
│   ├── tsconfig.json
│   ├── vite.config.ts              # configuração Vite + Vitest; `@vitest/coverage-v8` adicionado Sprint 34 com threshold 80% por glob em arquivos de lógica de negócio
│   ├── index.html                  # entry point, lang="pt-BR" (auditado via /impeccable audit)
│   ├── public/
│   │   └── fonts/                  # Archivo (600/700) + Public Sans (400/600), .woff2 auto-hospedado, licença OFL (Sprint 6); Inter (400/500/600/700/800) — sistema "Analyst Console" (Sprint 34)
│   ├── src/
...
│   │   │   ├── categoryColors.ts   # paleta categórica p/ Categoria (grupo) e Tipo (subcategoria) no funil — atribuição por id estável, nunca por ranking; Tipo deriva um color-mix() do grupo pai (Sprint 9, revisão pós-entrega); paleta expandida de 8 pra 16 cores (`--cat-1`..`--cat-16`, resolve colisão real Empréstimos/Transferência Interna com `i % 8`), `buildGroupColorIndex` generalizado pra `buildColorIndexFromIds(ids: number[])` — mesma função de índice reaproveitada por `AtivosAccordion`/`AssetsPage.tsx` pra colorir por ativo, não só por categoria/grupo (Sprint 24)
│   │   │   ├── transactionEdit.ts  # EditableTransaction (forma mínima compartilhada entre CategorizedTransaction/PluggyTransaction) + descricaoExibida (Sprint 10); +subcategoryLabel, extraída de TransactionEditCells/CategorizationReviewPage (Sprint 11); EditableTransaction +data/data_editada_manualmente (Sprint 18); EditableTransaction +investimento_id/investimento_sugerido_id, investimentoLabel novo (Sprint 19)
...
│   │   │   ├── resolveClickedPonto.ts  # helper: resolve click event de recharts → {ano, mes} (Sprint 26, reaproveita helper de Projeção removida na Sprint 30)
│   │   │   └── sharedChartDomain.ts  # função computeSharedDomain(series) — calcula domínio Y único compartilhado entre múltiplas séries para gráfico sobreposto (ReceitaDespesaSaldoChart do Dashboard, Sprint 34, novo); 100% testada
│   │   ├── api/
...
│   │   │   ├── pluggy.ts           # chamadas /pluggy/* (Sprint 3); apelido/sync_enabled em PluggyAccount, updatePluggyAccount, syncPluggyItems (Sprint 7); liabilityId em filtros, account_tipo em PluggyTransaction (Sprint 9); PluggyAccount +saldo_inicial, updatePluggyAccountSaldoInicial, fetch/updateSalarioAjusteDezembro (Sprint 15); PluggyTransaction +data_editada_manualmente (Sprint 18); PluggyAccount +investimento_id, updatePluggyAccount +investimentoId (full-replace — sempre enviado, mesmo em edição de apelido/sync), PluggyTransaction +investimento_id/investimento_sugerido_id, investimentoId em PluggyTransactionFilters (Sprint 19)
│   │   │   ├── investimentos.ts    # chamadas /investimentos/* — fetch/create/update/deleteInvestimento, fetchInvestimentoEvolucao (Sprint 19, novo); fetchEvolucaoMensal (Sprint 21); fetchInvestimentoTransacoes — extrato unificado, filtro {ano, mes} (Sprint 23, novo)
...
│   │   │   ├── TrendLineChart.tsx    # gráfico de tendência consolidado — 3 variantes (spark/row/card); tooltip + interatividade: hover mostra mês/valor, clique navega pra filtro ano/mes (Sprint 26)
...
│   │   │   ├── TransactionsTable.tsx  # tabela de transação unificada (Sprint 13) — substitui TransacoesPanel (DashboardsPage), o <table> hand-rolled de AssetDrilldown e o de LiabilityDrilldown; flags showCategoria/showAtivo (default true), colgroup .txn-table; +investimentoId/showInvestimento (default false) (Sprint 19); restilizada pra `.ac-txn-table` (Analyst Console) — cabeçalho denso/uppercase, célula de valor com `DirectionIcon` (função interna do arquivo, não um componente à parte) colorida via `transaction.tipo`, não o sinal de `valor` (evita regressão do achado NuTag da Sprint 10, ver TransactionTipoIcon.tsx) (Sprint 34)
...
│   │   │   ├── SubcategoryGroupTable.tsx  # tabela agrupada por categoria, primeira linha de cada grupo com borda superior mais forte (Sprint 30, extraído de NaturezaPage, revisão de design via Artifact)
│   │   │   ├── RegimeToggle.tsx      # toggle Competência (default)/Caixa, mesmo padrão aria-pressed do toggle despesa/receita de AssetsPage (Sprint 16, novo); prop `variant="ac"` para renderizar no sistema Analyst Console (backward compatible) (Sprint 34)
│   │   │   ├── KpiTile.tsx           # componente reutilizável KPI (label + delta vs. período anterior + valor + sparkline + selo de conferência) — usado 8x na página do Dashboard do Analyst Console (5 KPIs fluxo + 3 Ativos/Passivos/Patrimônio compact) (Sprint 34, novo)
│   │   │   └── ChartTooltip.tsx      # helper de tooltip/crosshair para gráfico de linha de série única — componente `ChartTooltip` + `chartCursorProps` (Sprint 34, novo); o Dashboard hoje só reaproveita `chartCursorProps` (ReceitaDespesaSaldoChart usa um tooltip multi-série próprio, `ComparativoTooltipContent`, local a DashboardsPage.tsx, porque `ChartTooltip` só mostra um valor por vez) — `ChartTooltip` em si fica disponível pra um gráfico de série única futuro do épico E10
│   │   ├── hooks/
...
│   │   │   ├── useUpdatePluggyAccount.ts # mutation PUT /pluggy/accounts/{id} — apelido/sync_enabled (Sprint 7); +investimentoId opcional, invalida investimentoEvolucao (Sprint 19)
...
│   │   │   ├── invalidateDashboardQueries.ts  # invalidateAfterTransactionEdit — invalida categorizationTransactions/pluggyTransactions + todo query "dashboard*"/"saldoPorConta" por predicate; usado pelas 4 mutations de edição de transação (Sprint 10); predicate extraído em invalidateAllDashboardQueries, reaproveitado por invalidateAfterSubcategoryEdit (Sprint 12); invalidateAfterSubcategoryEdit ganha invalidação de categoryGroups (antes só subcategories/dashboard*, faltava pro CRUD de grupo de CategoriasPage); invalidateAfterOrcamentoEdit novo — invalida orcamentos + dashboard* (Sprint 30)
...
│   │   │   ├── useEvolucaoSaldoPorConta.ts   # GET /dashboards/evolucao-saldo-por-conta (Sprint 15) — usado hoje só por AccountManagementPage (drill-down do Dashboard trocou pra useSaldoAcumuladoConferencia na Sprint 32)
│   │   │   ├── useDashboardSaldoAcumulado.ts # GET /dashboards/saldo-acumulado, pede periodoHistorico+1 (ponto extra pro card "Saldo Anterior") (Sprint 15); parâmetro regime, na queryKey (Sprint 16); parâmetro regime removido — fórmula nova não depende de regime (Sprint 32)
│   │   │   ├── useSaldoAcumuladoConferencia.ts  # GET /dashboards/saldo-acumulado/conferencia, tabela de conferência (Total + por conta corrente) do drill-down (Sprint 32, novo)
│   │   │   ├── useOrcamentos.ts              # lista orcamentos do usuário (Sprint 30, novo)
│   │   │   ├── useCreateOrcamento.ts         # mutation POST /orcamentos (Sprint 30, novo)
│   │   │   ├── useUpdateOrcamento.ts         # mutation PUT /orcamentos/{id} (Sprint 30, novo)
│   │   │   ├── useDeleteOrcamento.ts         # mutation DELETE /orcamentos/{id} — invalida orcamentos/dashboardPorOrcamento (Sprint 30, novo)
│   │   │   ├── useDashboardPorOrcamento.ts   # GET /dashboards/por-orcamento, agrega orçado-vs-realizado por subcategoria (Sprint 30, novo)
│   │   │   ├── useCreateCategoryGroup.ts     # mutation POST /category-groups (Sprint 30, novo)
│   │   │   ├── useUpdateCategoryGroup.ts     # mutation PUT /category-groups/{id} (Sprint 30, novo)
│   │   │   ├── useDeleteCategoryGroup.ts     # mutation DELETE /category-groups/{id} — invalida categoryGroups/subcategories/dashboard* via invalidateAfterSubcategoryEdit (Sprint 30, novo)
│   │   │   ├── useCreateSubcategory.ts       # mutation POST /subcategories (Sprint 30, novo)
│   │   │   ├── useUpdateSubcategory.ts       # mutation PUT /subcategories/{id} (Sprint 30, novo — antes só editava natureza)
│   │   │   ├── useDeleteSubcategory.ts       # mutation DELETE /subcategories/{id} — invalida categoryGroups/subcategories/dashboard* via invalidateAfterSubcategoryEdit (Sprint 30, novo)
│   │   │   ├── useInvestimentos.ts           # lista investimentos do usuário (Sprint 19, novo)
│   │   │   ├── useCreateInvestimento.ts      # mutation POST /investimentos (Sprint 19, novo)
│   │   │   ├── useUpdateInvestimento.ts      # mutation PUT /investimentos/{id} (Sprint 19, novo)
│   │   │   ├── useDeleteInvestimento.ts      # mutation DELETE /investimentos/{id} — invalida investimentos/pluggyAccounts/pluggyTransactions/dashboardPorInvestimento*/categorizationTransactions (Sprint 19, novo, mirror de useDeleteAsset)
│   │   │   ├── useInvestimentoEvolucao.ts    # GET /investimentos/{id}/evolucao (Sprint 19, novo)
│   │   │   ├── useInvestimentoGastos.ts      # GET /dashboards/por-investimento, com período+tipo (Sprint 19, novo, mirror de useAssetGastos)
│   │   │   ├── useInvestimentoGastosTendencia.ts  # GET /dashboards/por-investimento/tendencia, série por investimento (Sprint 19, novo, mirror de useAssetGastosTendencia)
│   │   │   ├── useSetTransactionInvestimento.ts   # mutation PUT /categorization/transactions/{id}/investimento (Sprint 19, novo, mirror de useSetTransactionAsset)
...
│   │   │   ├── useInvestimentoTransacoes.ts  # GET /investimentos/{id}/transacoes, extrato unificado conta+holding, filtro ano/mes (Sprint 23, novo)
│   │   │   └── useDeleteAccount.ts           # mutation DELETE /pluggy/accounts/{id} — invalida pluggyAccounts/pluggyTransactions/categorizationTransactions/evolucaoSaldoPorConta (Sprint 22, novo, mirror de useDeleteAsset)
│   │   └── pages/
│   │       ├── LoginPage.tsx       # botão "Entrar com Google" (link para /auth/google/login)
│   │       ├── ProtectedPage.tsx   # nome/e-mail do usuário + abas Início/Dashboards/Categorizar/Gestão de contas/Ativos (Sprint 5; aba Transações removida e Conectar conta renomeada na Sprint 7; aba Ativos na Sprint 8); aba Início removida (Dashboards vira a aba inicial), aba Passivos nova, Gestão de Contas move pro final — ordem: Dashboards/Categorizar/Ativos/Passivos/Gestão de Contas (Sprint 10); aba Natureza nova, entre Passivos e Gestão de Contas (Sprint 12); aba Projeção nova, entre Natureza e Gestão de Contas (Sprint 14); aba "Gestão de contas" vira "Configurações" (renderiza ConfiguracoesPage), última do menu (Sprint 15); aba Investimentos nova, entre Ativos e Passivos (Sprint 19); aba Projeção removida, aba Orçamento nova (Sprint 30), aba Categorias nova (Sprint 30); **sprint 34:** shell/sidebar completamente redesenhado no sistema visual "Analyst Console" — ícones de nav novos (Natureza/Orçamento/Categorias/Configurações), tokens `--ac-*` aplicados a todas as abas (sistema novo coexiste com o antigo)
│   │       ├── DashboardsPage.tsx  # filtro ano/mês, cards com sparkline, funil de drill-down em sanfona + Recharts (Sprint 5, sanfona/tendência/percentual na Sprint 6); cards Ativos/Passivos/Saldo clicáveis, funil de categoria expande direto pra transações (nível "meio de pagamento" removido, vira AccountTipoIcon por linha), tabelas ordenáveis por coluna (Sprint 9); funil de Despesa/Receita ganha um nível — Categoria (`GrupoAccordion`) > Tipo (`SubcategoriaAccordion`) > Transação — com cores via `categoryColors.ts`, ícone dentro da célula Valor, coluna % ordenável (revisão pós-entrega da Sprint 9); card Patrimônio clicável com `PatrimonioBreakdownPanel` (4 partes + total, link pros drill-downs de Ativos/Passivos/Saldo); `TransacoesPanel` ganha colunas Categoria/Ativo editáveis inline via `TransactionEditCells` (Sprint 10); `TransacoesPanel` exportada (antes privada) e reaproveitada como nível "transação" do funil de `NaturezaPage` (Sprint 12); `TransacoesPanel`/`SortableHeader` extraídos pra `components/TransactionsTable.tsx`/`SortableHeader.tsx` (unificação da tabela de transação), `Row` exportada pra reuso em `NaturezaPage`, `PatrimonioBreakdownPanel` ganha colgroup (Sprint 13); cards "Saldo Acumulado" (drill-down com `TrendLineChart`) e "Saldo Anterior" (primeiro card da grid, navega `ano`/`mes` ao clicar exceto em jan/2026, que alerta) — `useDashboardSaldoAcumulado` pede um mês a mais que o histórico selecionado, ponto extra alimenta "Saldo Anterior" sem chamada nova (Sprint 15); state `regime` levantado + `<RegimeToggle>` no filtro, propagado pra `GrupoAccordion`/`AtivosAccordion`/`PassivosAccordion`/`PatrimonioBreakdownPanel`; `PatrimonioBreakdownPanel` atualizado pros campos novos (`saldo_liquido_acumulado`/`saldo_investimentos`), "Ver detalhe" de Saldo líquido acumulado abre o drill-down de Saldo Acumulado (Sprint 16); card "Saldo Acumulado" ganha `.tag` + drill-down ganha nota explicando que é projeção por competência (não snapshot bancário) — Bloco 2 da Sprint 18, investigação com dado real confirmou diferença conceitual (salário no fim do mês deferido pra competência do mês seguinte), não bug (Sprint 18); drill-down "Ativos" ganha seções `InvestimentosValorAtualList`/`SaldoPorContaList` ao lado do accordion de gasto; `PatrimonioBreakdownPanel` troca os 3 destinos de "Ver detalhe" (Ativos/Passivos/Saldo em investimentos) de gasto do período/lista sem filtro pra listas itemizadas de valor atual (`AssetsValorAtualList`/`LiabilitiesValorAtualList`/`InvestimentosValorAtualList`, 3 `DrillKind` novos `patrimonioAtivos`/`patrimonioPassivos`/`patrimonioInvestimentos`, distintos dos `ativos`/`passivos` usados pelos cards de topo) — fórmula somada dos cards não mudou (Sprint 22); grid vira 2 `.dash-summary` (1ª linha Ativos/Passivos/Patrimônio, 2ª linha o resto), disclaimers `.tag` removidos de Saldo Acumulado/Patrimônio; `mesSeguinte` (inverso de `mesAnterior`) + seta real `<button>` aninhada no tile de Saldo Acumulado (que vira `<div role="button">` com `onKeyDown` — dois `<button>` aninhados é inválido em HTML), seta decorativa em Saldo Anterior; card "Saldo" troca `SaldoPorContaList` por `SaldoMemoriaCalculo` (Receita−Despesa=Saldo, sem endpoint novo); drill-down de Saldo Acumulado ganha `SaldoAcumuladoMemoriaCalculo` (âncora via `useEvolucaoSaldoPorConta` + tabela mês-a-mês, acima do `TrendLineChart` já existente; `TrendLineChart` ganha interatividade de clique → filtro ano/mes em todos os drill-downs (Sprint 26)); `InvestimentosValorAtualList` vira accordion Investimento→Holding (`InvestimentoHoldingsList`, busca sob demanda via `usePluggyInvestments(id)` só ao expandir), reaproveitado tanto no card Ativos quanto dentro do accordion de Patrimônio; card Passivos ganha `LiabilitiesValorAtualList` ao lado do accordion de gasto; os 3 `DrillKind` `patrimonioAtivos`/`patrimonioPassivos`/`patrimonioInvestimentos` removidos — `PatrimonioBreakdownPanel` vira accordion de 4 partes expansível in-place (sem navegação, reaproveita `saldoAcumuladoSparkline` já calculado no componente pai em vez de nova query); `AtivosAccordion` colore por ativo (`buildColorIndexFromIds` sobre `useAssets()`, não mais `var(--despesa)`/`var(--receita)` fixo) (Sprint 24); `SaldoAcumuladoMemoriaCalculo` (âncora + tabela mês-a-mês) removida, substituída por `SaldoAcumuladoConferenciaTable` — tabela `.dash-table` sempre visível (sem acordeão) com Total (100%) + uma linha por conta corrente via `useSaldoAcumuladoConferencia`; `useDashboardSaldoAcumulado` chamado sem `regime` (Sprint 32); **Sprint 34 — Redesign "Analyst Console":** topo da página completamente migrado pro novo sistema — 5 KPIs de fluxo (Saldo Anterior, Receita, Despesa, Saldo, Saldo Acumulado) com delta vs. mês anterior + sparkline usando `KpiTile`, row secundária Ativos/Passivos/Patrimônio também migrada (mesmo `KpiTile`, modo `compact`, sem delta/sparkline — sem endpoint de série histórica pra esses 3 totais, decisão do CEO de não expandir o backend nesta sprint), tabela de conferência `SaldoAcumuladoConferenciaTable` sempre visível (sem acordeão, classe `.ac-table`), gráfico combinado `ReceitaDespesaSaldoChart` (3 séries Receita/Despesa/Saldo Acumulado sobrepostas na mesma escala via `computeSharedDomain`, tooltip multi-série próprio `ComparativoTooltipContent`) substituindo o comparativo em pequenos múltiplos original, navegador de mês (◀ ▶) respeitando limite mês corrente; só o funil de drill-down (categoria→transação, abaixo do "Fechar") permanece no sistema antigo — transição progressiva do épico E10; `TransactionsTable` usada dentro desse funil (Dashboard/Ativos/Passivos) restyled (`.ac-txn-table` classe descendente) com cabeçalho denso/uppercase, valor com seta de direção colorida por `transaction.tipo`, ainda editável inline
│   │       ├── AccountManagementPage.tsx  # Gestão de Contas — lista contas conectadas, apelido/sync_enabled editáveis, diálogo "Sincronizar MeuPluggy" com pré-seleção (Sprint 7, renomeado de ConnectAccountPage.tsx); listas ganham classe `.simple-list` (espaçamento/hover, sem virar accordion) (Sprint 13); campo "Saldo inicial (31/12/2025)" editável inline por conta + tabela de auditoria mensal (`.dash-table`), reaproveitada dentro de ConfiguracoesPage (Sprint 15); `<select>` de vínculo carteira→investimento inline, só pra linhas `tipo=investimento` — edição de apelido/toggle de sync passam a enviar `investimento_id` explícito (PUT é full-replace) pra não desvincular a carteira sem querer (Sprint 19); seção nova "Posições de investimento" — lista holdings sincronizadas (mirror da lista de carteiras: `<select>` de vínculo a Investimento + saldo inicial editável inline) (Sprint 20); botão "Excluir conta" por linha, habilitado só quando `sync_enabled=false`, chama `DELETE /pluggy/accounts/{id}` via `useDeleteAccount` (Sprint 22, novo)
│   │       ├── CategorizationReviewPage.tsx  # listagem única de transações (substitui TransactionsPage) — filtro tipo/status, lote, categoria editável em confirmada, descrição inline + propagação (Sprint 4; paginação pós-Sprint 6; rework completo na Sprint 7); filtros novos "associado a ativo"/"categoria" (grupo), indicador visual débito/crédito (`TransactionTipoIcon`) por linha, descrição/ativo passam a usar `TransactionEditCells` compartilhado (categoria continua bespoke — fila de pendentes tem workflow de confirmação em lote que não se aplica ao auto-save do componente compartilhado) (Sprint 10); `<select>` de categoria trocado por `CategoryCombobox` (mesmo estado local bufferizado até confirmação), status vira `StatusIcon` (ícone, revisão pós-implementação — era badge de texto na primeira versão), colunas reordenadas (Status/Data/Descrição/Categoria/Ativo/Valor), `cat-review-table` ganha densidade maior (padding reduzido) e larguras por coluna (mais espaço pra Descrição/Categoria, menos pro resto) (Sprint 11); ganha sort (Data/Descrição/Valor) via `SortableHeader` compartilhado — a tabela "flagship" da Sprint 11 nunca tinha tido (Sprint 13); coluna Data estática vira `<DateCell />` editável (Sprint 18); coluna Investimento nova (`InvestimentoSelectCell`), sort incluído (Sprint 19); botão "Sincronizar contas" na barra de filtros — `useSyncPluggyItems().mutate(undefined)` direto, sem diálogo de seleção de conta (reaproveita a mutation já usada em Gestão de Contas, que já invalida `categorizationTransactions`) (Sprint 24)
│   │       ├── AssetsPage.tsx      # Gestão de Ativos — grid de cards ativos/baixados, CRUD (criar/editar/vender/deletar), toggle despesa/receita, sparkline por card, drill-down (painel fora do card, gráfico de histórico + transações) por ativo, filtro período (Sprint 8; refatorada na Sprint 9 pra reaproveitar sparkline/gráfico compartilhados; consolidação em `TrendLineChart` com variantes (spark/card) e interatividade de clique (Sprint 26)); `AssetDrilldown` migrado pra `TransactionsTable` (`showAtivo={false}`) — ganha Categoria editável + sort, que não tinha antes; botões Editar/Vender viram `.btn-ghost`, Excluir vira `.btn-ghost.btn-quiet.btn-danger` — só "Ver gasto no período" fica Default (Sprint 13); state `regime` + `<RegimeToggle>` ao lado do toggle despesa/receita (Sprint 16); sparkline do card e `AssetDrilldown` coloridos por ativo (`buildColorIndexFromIds`/`groupColorVar` de `categoryColors.ts`), não mais `var(--despesa)`/`var(--receita)` fixo por tipo de transação (Sprint 24)
│   │       ├── InvestimentosPage.tsx  # Gestão de Investimentos — clone estrutural de AssetsPage: grid de cards (saldo atual, rendimento estimado rotulado como estimativa, carteiras vinculadas), CRUD (nome só), drill-down fora do card com toggle Aporte/Resgate + sparkline por card, filtro período (Sprint 19, novo — sem toggle Competência/Caixa); card ganha tag "Posições: <código/nome>"; drill-down ganha toggle "Extrato"/"Posições" — view "Posições" (`InvestimentoPosicoes`) lista holdings vinculadas em tabela com linha expansível (`PosicaoHistorico`) mostrando o histórico via `GET /pluggy/investments/{id}/transactions` (Sprint 20); card perde as tags soltas "Carteiras:"/"Posições:" (poluição visual), botão do card abre o drill-down direto na view "Posições" (era "Extrato"); aba "Extrato" troca a fonte pro endpoint unificado `useInvestimentoTransacoes` (`GET /investimentos/{id}/transacoes`, une `PluggyTransaction` + `PluggyInvestmentTransaction` — corrige investimento só-holdings como "Quitar o AP" que ficava sempre vazio), tabela nova `extrato-unificado-table` com `<colgroup>`; `InvestimentoPosicoes`/`PosicaoHistorico` ganham `<colgroup>` (eram as únicas `.dash-table` do projeto sem, causando overlap de coluna com nome de holding longo) (Sprint 23)
...
│   │       ├── NaturezaPage.tsx    # tela "Natureza" — dashboard de visibilidade (3 cards Fixo recorrente/Variável recorrente/Eventual, drill-down reaproveitando /por-categoria+useSubcategories) + tabela de classificação de subcategorias agrupada por CategoryGroup (rowSpan, table-layout:fixed), <select> de 3 opções salvando via PUT /subcategories/{id} (Sprint 12 — reaproveita Subcategory.natureza dormente desde a Sprint 2, sem migration); rótulo "Custo eventual"→"Eventual", funil ganha o nível Categoria (`Natureza → Categoria → Subcategoria → Transação`, sanfona multi-nível via `categoriaGrouping.ts`), tabela de classificação ganha sort de Categoria/Subcategoria (Sprint 13); migrada pra usar `SubcategoryGroupTable` compartilhado (Sprint 30)
│   │       ├── OrcamentoPage.tsx   # tela nova "Orçamento" — CRUD de orçamentos por subcategoria (toggle eventual/recorrente com campos condicionais via `CategoryCombobox` + `.dash-toggle`), listagem `.simple-list` agrupada por subcategoria, mesmo padrão de form inline de AssetsPage (Sprint 30, novo)
│   │       ├── CategoriasPage.tsx  # tela nova "Gestão de Categorias" — CRUD de grupos e subcategorias (criar/renomear/mover/deletar, bloqueio com mensagem se em uso), tabela `SubcategoryGroupTable` compartilhado com NaturezaPage (Sprint 30, novo)
...
│       ├── check-sprint22.mjs      # fila de Categorização sem proventos de investimento, botão "Excluir conta", série histórica de "Quitar o AP" sem pico, drilldowns novos de Ativos/Patrimônio; só navega/fotografa, não muta (Sprint 22)
│       └── check-sprint34.mjs      # Analyst Console — Dashboard topo novo (5 KPIs + navegador mês + tabela conferência + gráfico combinado), compare vs. mockup aprovado, claro/escuro/desktop/mobile, validação dos 3 bugs de layout corrigidos durante a sprint (sparkline width, KPI grid mobile, cabeçalho Saldo Acumulado flex-wrap) (Sprint 34, novo)
├── .github/
│   └── workflows/
...
- Orçamento sugerido automático (ex. baseado em média histórica) — fica como candidato de sprint futura se o CEO priorizar (Sprint 30).
