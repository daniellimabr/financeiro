...
│   │   │   └── naturezaLabels.ts   # única fonte dos 3 rótulos de natureza (fixa/variavel/eventual→"Custo eventual" default) + var CSS de cor por natureza (Sprint 12)
│   │   ├── api/
...
│   │       └── NaturezaPage.tsx    # tela "Natureza" — dashboard de visibilidade (3 cards Fixo recorrente/Variável recorrente/Custo eventual, drill-down natureza→subcategoria→transação reaproveitando /por-categoria+useSubcategories) + tabela de classificação de subcategorias agrupada por CategoryGroup (rowSpan, table-layout:fixed), <select> de 3 opções salvando via PUT /subcategories/{id} (Sprint 12 — reaproveita Subcategory.natureza dormente desde a Sprint 2, sem migration)
...
│       └── check-sprint12.mjs      # 3 cards de natureza, drill-down natureza→subcategoria→transação, toggle despesa/receita, tabela de classificação; edita a natureza da 1ª subcategoria real (única mutação), restaurada por PUT direto em `finally` a partir do valor capturado via GET /subcategories antes do clique — preserva null vs. "eventual" explícito, indistinguíveis no <select> (Sprint 12)
├── .github/
│   └── workflows/
