...
| E12 | Planejamento financeiro (em progresso) | Planejamento de fluxo futuro com 10 colunas (3 meses passados + 1 corrente + 6 futuros) por subcategoria com natureza fixa/variável, linha-lembrete "Eventual" (agrega natureza eventual, editável a partir do mês corrente), sugestão automática via média de 3 meses (arredondada a 3 casas significativas), edição inline com propagação até o fim do horizonte, linha "Saldo simulado" por seção — ✅ Sprint 38 (2026-09-11, aprovada 2026-09-12, com 4 rodadas de ajuste pós-deploy); sucessor de E9 (Projeção, removida em Sprint 30) e do mecanismo de Orçamento (removido nesta sprint após 8 sprints de uso). Próximo candidato: linha de projeção de saldo acumulado (PRD-040/SPRINT-040, proposto, execução pendente) |
...
### ✅ Sprint 38 — Mesa de Planejamento (E12, abre o épico) concluída em 2026-09-11, aprovada pelo CEO em 2026-09-12
...
688 testes backend (100% cobertura em `app/planejamento`, 99% total), 295 testes frontend, tsc/eslint/ruff/prettier limpos na execução original.

**Fase 5 (QA visual + deploy)**: `check-sprint38.mjs`, deploy na VM de dev, migrations `0022`/`0023` aplicadas.

**Ajustes pós-deploy (feedback real do CEO, 2026-09-12)** — 4 rodadas na mesma tela, sem sprint numerada nova: (1) horizonte reduzido de 12 para 6 meses futuros (16→10 colunas, não cabia sem scroll horizontal); confirmar sugestão passa a propagar pros meses seguintes; linha-lembrete "Eventual" nova (agrega subcategorias natureza `eventual`, média móvel, conta no total); totais movidos pro backend; linha "Saldo simulado" nova; Receitas antes de Despesas — commit `6a574ec`. (2) linha Eventual tinha ficado 100% só-leitura por ambiguidade do pedido — corrigida pra aceitar override nos meses futuros, nova tabela `planejamento_valores_eventual` (migration `0024`) — commit `792bc64`. (3) mês corrente da linha Eventual também liberado pra edição — commit `5c036de`. (4) bug real de propagação (só cobria o fim do horizonte quando editado a partir da 1ª coluna futura) corrigido pra sempre propagar até a última coluna exibida — commit `55b6e78`. (5) sugestões calculadas passam a arredondar pra 3 casas significativas (1281,89→1280 etc.) — commit `7df2de0`. Estado final: 712 testes backend (99% cobertura), 303 testes frontend. Relatório completo em [SPRINT-038-mesa-de-planejamento-report.md](sprints/SPRINT-038-mesa-de-planejamento-report.md), seção "Ajustes pós-deploy".
...
- **Indicador de status de conciliação na sidebar (Analyst Console):** o mockup aprovado do redesign
  "Analyst Console" tem um rodapé de sidebar mostrando quantas contas correntes estão conferidas
  (ex.: "3 de 3 contas"). Não implementado na Sprint 34 — precisa de hook/endpoint novo que não
  existe ainda. Épico E10 fechou na Sprint 36 sem essa peça; candidata a sprint futura própria (fora
  de épico), sem PRD/plano ainda.
- **Sparkline/delta dos KPIs Ativos/Passivos/Patrimônio (Analyst Console):** decisão explícita do CEO
  durante a execução da Sprint 34 (2026-08-21) — o mockup mostra sparkline nesses 3 tiles, mas não
  existe endpoint de histórico pra esses totais hoje (só o snapshot atual via
  `usePatrimonioBreakdown`); o CEO escolheu não expandir o backend nesta sprint, então os 3 tiles
  usam `KpiTile` em modo `compact` (label + valor, sem delta/spark). Épico E10 fechou na Sprint 36
  sem essa peça; candidata a sprint futura própria se o CEO priorizar um endpoint de série histórica
  de patrimônio.
