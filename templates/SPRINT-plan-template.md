# SPRINT-NNN: {Nome} — Plano

- **PRD(s):** {links}
- **Data do plano:** {AAAA-MM-DD}

## Objetivo da sprint

Uma frase: o que muda para o usuário ao final.

## Tarefas

| # | Tarefa | Subagente/modelo | Arquivos/docs de contexto |
|---|---|---|---|
| 1 | ... | Sonnet: implementação | {doc relevante} |
| 2 | ... | Haiku: doc-updater | {doc relevante} |

## Testes previstos

Unitários: quais regras de negócio. Integração: quais endpoints.

## Impacto no roadmap

Épico afetado, sprints seguintes impactadas (se houver).

## Riscos / dependências

O que pode bloquear ou exigir decisão do CEO no meio da sprint.
